...
This page covers lifecycle stage 7 of the [Production memory](../README.md) track: Index.
...
and no arrow points back. That one-way flow is the rule this stage exists for.
...
From stage 8 on every write ends in a rebuild, so this is not a rare accident: whoever writes last
...
The profile view is where stage 5's claim key (the stable name a fact updates) pays off a second time.
...
This stage's src only provides the verb: nothing calls `reindex` on its own.
From stage 8 on the write path calls it: every pass that changes records reindexes its scope,
...
Since neither precondition holds, this stage builds neither.

### What Changed

Compared with stage 6, indexing and retrieval are now separate concerns.
This stage builds channels that return candidates. Stage 8 decides how to rank across them,
...
- **Index used to decide relevance.** A channel returns candidates; it does not rank the final answer.
  Fusion is stage 8, and folding it in here makes both stages untestable.
...
- [Section 9 · Memory](https://github.com/hardness1020/awesome-agent-architecture/tree/main/sections/09-memory/): the single index this stage splits into channels.
