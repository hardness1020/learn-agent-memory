...
這一章講 [Production memory](../README.zh-TW.md) track 的階段 7（Index）。
view 指的是從紀錄整理出來的讀取結構，搜尋用的 index、看當下狀態的 profile、
互相連結的 wiki 頁面都是。每一種都從同一份 ledger 衍生出來。

走到這個階段，一筆紀錄已經有類型、有時間軸、也有合併的歷史，但它終究只是一列資料。
不同的問題，要的答案長得完全不一樣。
...
這個單向流動就是這個階段要守的規則。index 壞掉不算事故，重建就好：
...
階段 8 起每次寫入跑完都會重建一次，所以這不是偶發事故：誰最後寫，表裡就只剩誰的資料。
...
profile 是階段 5 的 claim key（一個事實更新時固定用的那個名字）第二次派上用場的地方。
沒有 key 就沒有投影的目標，「當下狀態」就會退化成「最近那幾筆 memory」。

資料的流向：紀錄進去，view 出來，engine 隨時可以重建任何一種。
`reindex` 拿一個 scope 還生效的紀錄，把 sparse view 從頭重寫一次。
這個階段的 src 只給出 `reindex` 這個動作，沒有任何路徑會自動呼叫它。
到了階段 8，換寫入路徑來呼叫：任何改動紀錄的流程跑完就 reindex 自己的 scope，retrieve 只讀表。
...
這兩個前提都不成立，所以這個階段兩種都沒做。

### What Changed

和階段 6 相比，建 index 和做檢索分成兩件事。
這個階段建出回傳 candidate 的各個通道，階段 8 才決定怎麼跨通道排序。
...
- [第 9 章 · Memory](https://github.com/hardness1020/awesome-agent-architecture/tree/main/sections/09-memory/README.zh-TW.md)：那裡的單一 index，這個階段拆成多個通道。
