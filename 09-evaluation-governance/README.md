...
This page closes the [Production memory](../README.md) track: lifecycle stage 10, Feedback and evaluation.
The first nine stages write, resolve, merge, index, rank, and inject, and none of them ever finds out
whether it was right. This stage adds the feedback that closes that loop.
...
Evaluation stores nothing of its own. Every metric is computed on read, from stage 2's ledger and the records,
under the same rule stage 7 applies to indexes: drop the numbers, recompute them, and nothing is lost.
...
`used` and `misled` are judgments, and they have to come from outside this stage.
...
Data flow through the stage: `recall` logs what it selected before it returns the block, the caller reports
...
one per segment of the pipeline. Each metric is computable only because an earlier stage logged something:

| Layer | Metrics | What it measures | Computed from | Notes |
| --- | --- | --- | --- | --- |
| write | decisions · store rate · duplicate rate · approval queue | what the gate decided, and the review backlog | stage 3's decision log, with the rule that decided | |
...
| | unsupported memory rate | cites evidence the ledger lacks | stage 4's source event ids | not *unsupported claims*: that is about the answer, not measured here (see below) |
| | wrong update rate | stored memories later retracted as wrong | stage 5's retract and supersede operations | |
| | consolidation precision | how many merge proposals survived validation | stage 6's returned rejections | |
| retrieval | evidence precision · misled rate | how many retrieved memories helped or misled | live outcomes | |
| | contradiction coverage | did both sides of a conflict go in | stage 9's conflict groups | group-atomic: a turn is 1.0 or 0.0; under 1.0 a conflict was dropped whole |
| | recall@k | how many of the right memories came back in the top k | a labeled case set, or not at all | |
| context | injected tokens · memories per turn | how much prompt space memory takes per turn | stage 9's per turn injection log | |
...
| | stale rate | injected memories that were already closed | stage 5's superseded_at, read through as_of | checkable because the row was stamped, not deleted |
| end to end | turns · correction rate · abstention rate | turns, user corrections, answers declining memory | outcomes | |

### Deliberately not measured

Some things this stage deliberately does not measure. Not oversights: each is missing one input
...
| **Whose property** | retrieval's: not worth the budget, stage 9 working as designed | the model's reasoning, reported in the outcome like any other |
...
has one operation waiting for it, `retract`, which is stage 5's vocabulary and not a new verb: the record
is marked wrong and stays readable, because retracted means it never was true while superseded means
the world moved on. It is the one state change this stage makes, and without a caller the metric that
...
Compared with stage 9, `assemble` now hands back what it chose along with what it rendered,
...
- **Only end-to-end numbers.** One score that moves for ten reasons cannot name the stage that broke.
  Score the layers separately, and let the failing layer point at its stage.
...
- **Erasure treated as a design bug.** Append-only meets a legal delete eventually. Handle it as one audited,
  scoped operation with a record of its own, not as a `DELETE` verb any stage may call.
...
- [`engine.py`](src/engine.py): `recall` logs what it injected including the empty turns,
  ...
  memories to report on, `retract` marks a memory wrong through stage 5's operation, and `evaluate`
...
```bash
python tracks/production-memory/09-evaluation-governance/src/test.py   # offline checks, no key
```
