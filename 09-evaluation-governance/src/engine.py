...
The contract Protocol stays at three verbs, and stage 10 adds none. What it
adds to the engine is one observation (feedback), one read (evaluate), and one
governance operation (retract) that goes through stage 5's existing
...
class Engine:
    ...
    def propose(self, scope, candidate, epistemic_type="fact") -> Decision:
        ...
        # The action is a metadata field, not a prefix on the message. Stage 10
    ...
    def recall(self, scope, query, budget=BUDGET) -> str:
        ...
        an injection nobody recorded is a turn stage 10 cannot score. It is a
    ...
    def feedback(self, scope, outcome) -> str:
        """Stage 10's write side, and it is not a fourth verb: an outcome is an
    ...
    def retract(self, scope, memory_id, reason) -> str:
        """Mark a memory wrong. Stage 5 separates this from supersede: a
        ...
        This is stage 10's one write that changes state, and it is what closes
    ...
    def evaluate(self, scope, cases=()) -> dict:
        """Stage 10's read side: four layers computed from the ledger and the
    ...
    def _log_op(self, scope, op) -> str:
        ...
        metadata for the same reason the decision action is: stage 10 counts
