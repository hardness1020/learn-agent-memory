...
class Candidate:
    ...
    claim_key: str = ""                # what this is a claim about (stage 5)
...
class Decision:
    ...
    reason: str
    confidence: float
    rule: str = ""          # which rule decided, for stage 10. "" means the
...
def _duplicate(c, existing):
    if c.claim_key:
        ...
        # wording is a duplicate; anything else is a correction for stage 5.
...
def decide(candidate, existing=(), classifier=None) -> Decision:
    ...
    for rule in RULES:
        if decision := rule(candidate, existing):
            # stamped here, once, instead of in every rule: stage 10 counts
...
def resembles(text, other) -> float:
    ...
    stored text while consolidation (stage 6) compares two stored records, and
