...
lifecycle 的阶段 6（Consolidate），把 event 变成知识。
...
阶段 5 只处理了其中很窄的一块（新主张关掉旧主张），剩下的都在这个阶段，
而且这是第一次由模型决定已经存在的 memory 该怎么处置。

难就难在这里。在这之前，每个阶段都只会新增记录。
...
被退回的 Proposal 会报告出去，不会被吞掉，因为阶段 10 要靠这些数字算 consolidation 的精确度。
...
这两个 bug 都出在更早的阶段，但要等这个阶段跑起来、跟前面串在一起，才看得出来。
照这条 track 的惯例，修正落在这个阶段带着走的那份 code，
前面的文件夹保留当时的版本，对照着看就知道整合改了什么。

**Bug 1：`DEFER` 没有地方可去。**
write gate 会把跟现有 memory 很像的 candidate 延后处理，
理由写着「已经有相似的 memory，交给 consolidation 合并」。
但被延后的 candidate 根本没存下来，consolidation 也就永远看不到它：这个阶段合并得动的每一组，gate 早就先丢掉了。
...
gate 判断像不像用一种算法，这个阶段分组的时候用的是另一套。
同一对记录，gate 算出来够像，所以把 candidate 延后过来；
这个阶段再算一次却不够像，达不到合并的门槛。
结果 gate 交过来的东西全部躺在存储区里，一笔都没被合并过。
修法是这个阶段直接用 gate 的函数和门槛，两边量出来的永远是同一个数字：
...
两个阶段要交接，得先对「像不像怎么算」有共识，不然一边说够像，另一边永远算出不够像。

### What Changed

跟阶段 5 比：`consolidate()` 是 contract 的第三个动词，现在有实现了。
`observe()` 和 `consolidate()` 都能离线跑完整条路；`recall()` 要等阶段 8。
...
模型要接进来，就是从 proposer 这个位置接。默认的 proposer 是确定性的，所以这个阶段没有 `demo.py`。
