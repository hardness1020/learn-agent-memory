...
This page covers lifecycle stage 3 of the [Production memory](../README.md) track, the Write gate:
...
[Section 9](https://github.com/hardness1020/awesome-agent-architecture/tree/main/sections/09-memory/) gated writes with a type taxonomy inside the extractor prompt:
only non-derivable facts of four types get saved. At production scale the gate must do more:
...
because merging near-duplicates is consolidation's job (stage 6), not the gate's:
...
deterministic validator
    ↓ commit
typed memory (stage 4) · the decision logged as an event
```

How it integrates: candidates come from fresh ledger rows (stage 2), on the warm path (run end, not per query).
Stored candidates become typed records (stage 4), deferred ones wait for consolidation (stage 6),
and approval-gated ones sit in a queue, the pattern Hermes uses for staged writes (section 9).
Decisions land back in the ledger as events, so stage 10 can measure write precision from replayable data.
...
In section 9, selection lived inside an extractor prompt and returned files or nothing.
...
- **The gate is too loose.** Recall noise climbs. This is measurable: write precision and duplicate rate (stage 10).
...
```bash
python tracks/production-memory/02-write-policy/src/test.py   # offline checks, no key
```
