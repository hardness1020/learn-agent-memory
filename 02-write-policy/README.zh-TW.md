...
這一章是 [Production memory](../README.zh-TW.md) track 的第三章：lifecycle 的階段 3（Write gate）。
...
[第 9 章](https://github.com/hardness1020/awesome-agent-architecture/tree/main/sections/09-memory/README.zh-TW.md)的做法，是把一套類型規則放進 extractor 的 prompt：
...
中等重疊就先擱著：把相近的 memory 合併是 consolidation（階段 6）的工作，不是這道 gate 的工作：
...
怎麼跟其他階段接起來：candidate 來自 ledger 新進的資料（階段 2），在 warm path 上做（執行結束才跑，不是每次查詢都跑）。
存下來的 candidate 變成有型別的紀錄（階段 4），擱著的等 consolidation 處理（階段 6），
要核准的排進佇列等人審，跟 Hermes 暫存寫入等核准是同一個模式（第 9 章）。
每個決定也會以 event 的形式寫回 ledger。階段 10 就靠這些可重放的資料，量出 write precision。
...
跟第 9 章比：以前選擇藏在 extractor 的 prompt 裡，結果不是寫了檔案，就是什麼都沒有。
...
- **gate 太鬆：**recall 的雜訊越來越多。這量得出來：write precision 和重複率（階段 10）。
...
- [Production memory track](../README.zh-TW.md)：這個階段所在的完整 lifecycle。
