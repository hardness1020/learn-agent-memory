...
这一章是 [Production memory](../README.zh-CN.md) track 的第三章：lifecycle 的阶段 3（Write gate）。
...
[第 9 章](https://github.com/hardness1020/awesome-agent-architecture/tree/main/sections/09-memory/README.zh-CN.md)的做法，是把一套类型规则放进 extractor 的 prompt：
...
中等重叠就先搁着：把相近的 memory 合并是 consolidation（阶段 6）的工作，不是这道 gate 的工作：
...
怎么跟其他阶段接起来：candidate 来自 ledger 新进的数据（阶段 2），在 warm path 上做（执行结束才跑，不是每次查询都跑）。
存下来的 candidate 变成有类型的记录（阶段 4），搁着的等 consolidation 处理（阶段 6），
要批准的排进队列等人审，跟 Hermes 暂存写入等批准是同一个模式（第 9 章）。
每个决定也会以 event 的形式写回 ledger。阶段 10 就靠这些可重放的数据，量出 write precision。
...
跟第 9 章比：以前选择藏在 extractor 的 prompt 里，结果不是写了文件，就是什么都没有。
...
- **gate 太松：**recall 的噪声越来越多。这量得出来：write precision 和重复率（阶段 10）。
