"""Memory contract (production memory track, stage 1: Scope, plus the engine surface).

The root of the runnable chain. Every later stage carries this file forward:
01 gives observe() its storage, 02 gates it, 03 types it, 07 indexes it,
08 completes recall(). consolidate() arrives with stage 05.
...
Two things are contractual before any mechanism exists:
  ...
  engine : the whole subsystem behind three verbs. observe (raw evidence in),
           ...
           maintenance). callers never see the ten stages behind them.
