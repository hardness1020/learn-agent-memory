...
lifecycle 的阶段 2（Capture），一份只加不改的 event log，后面每个阶段都从这里读数据。

extraction 一定会漏：模型挑「什么值得记」的时候，会挑错、也会漏掉细节。
如果系统只留提炼过的 memory，错一次就永远错了，因为没有原始记录可以回头核对。

[第 9 章](https://github.com/hardness1020/awesome-agent-architecture/tree/main/sections/09-memory/README.zh-CN.md)已经有迷你版：`log_run` 在运行结束时把整轮文字存进 SQLite，
...
- 第 9 章用 session 当 key。ledger 改用 scope 当 key，很多 tenant 共用一张表，但谁也读不到别人那几笔。
- `event_type` 让 ledger 收得下聊天以外的证据：tool 结果、纠正、批准、任务成败都算。
- `occurred_at` 和 `recorded_at` 把世界时间和系统时间分开，阶段 5 的时间推理就建在这上面。
...
- **世界时间用猜的：**`occurred_at` 乱填，阶段 5 的时间推理会被污染。不知道就留空。
...
- [第 9 章 · Memory](https://github.com/hardness1020/awesome-agent-architecture/tree/main/sections/09-memory/README.zh-CN.md)：`log_run` 和 `search_sessions`，迷你版的 ledger。
