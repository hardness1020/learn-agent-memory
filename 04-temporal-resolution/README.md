...
This page covers lifecycle stage 5 of the [Production memory](../README.md) track, Resolve: identity, conflict, and time.

Stage 4 gave every record a type and a source. It did not say what happens when a new record disagrees with an old one.
...
Every operation is non-destructive. There is no free-form `UPDATE` and no `DELETE`, here or in the event ledger (stage 2):
...
Data flow: the write gate (stage 3) passes a candidate, stage 4 constructs the typed record,
and this stage places it on the timeline and returns the operations performed.
The engine writes each operation back to the ledger as an event, so the timeline is itself replayable.
Retrieval (stage 8) filters to active records, and assembly (stage 9) reads the conflicts this stage recorded
and prints both sides rather than silently picking one.

### Two bugs found at integration

Both bugs live in the write gate (stage 3), and both stayed invisible until this stage arrived
and real corrections started reaching the gate. They broke the first version of this stage.
...
the gate was deferring exactly the case this stage exists to resolve.
...
In stage 4, a record was typed, validated, and then never touched.
...
- [Section 9 · Memory](https://github.com/hardness1020/awesome-agent-architecture/tree/main/sections/09-memory/): the single-clock store this stage extends.
