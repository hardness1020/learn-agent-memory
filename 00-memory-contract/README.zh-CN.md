...
lifecycle 的阶段 1（Scope），加上核心抽象里的 engine 接口。
这里写的 `contract.py`，后面每个阶段都会继续沿用。

memory 系统最严重的事故不是检索出错，而是 scope 出错：
A 用户的个人数据，出现在 B 用户的对话里。
这种问题靠调检索没有用：内容抓得越准，泄露就越严重。

第二个决定是接口。memory 背后有十个阶段在跑，但 harness 用起来只该有三个动词。
...
每个阶段的文件夹都带着这个文件和之前的所有代码，在同一个 `engine.py` 上逐步演化，
就像 sections 的 `loop.py` 一章一章长大。
diff 相邻两个阶段的 `src/`，看到的就是那个阶段的机制。
...
- **只有接口、没有保证：**三个动词本身什么都不保证。
  合约真正的内容是那条铁律（事件留着、view 可重建），后面每个阶段的测试就是在逐一验它。

---

## 可执行程序

[`src/`](src/) 是整条链的起点，后面每个阶段都会带着它往下走：
...
- [第 9 章 · Memory](https://github.com/hardness1020/awesome-agent-architecture/tree/main/sections/09-memory/README.zh-CN.md)：Claude Code（按项目）和 Hermes（按用户）的 scope 做法。
- [Production memory track](../README.zh-CN.md)：这一章实现的阶段 1 和核心抽象。
