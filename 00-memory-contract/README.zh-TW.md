...
lifecycle 的階段 1（Scope），加上核心抽象裡的 engine 介面。
這裡寫的 `contract.py`，後面每個階段都會繼續沿用。

memory 系統最嚴重的事故不是檢索出錯，而是 scope 出錯：
A 使用者的個人資料，出現在 B 使用者的對話裡。
這種問題靠調檢索沒有用：內容抓得越準，洩漏就越嚴重。

第二個決定是介面。memory 背後有十個階段在跑，但 harness 用起來只該有三個動詞。
...
每個階段的資料夾都帶著這個檔案和之前的所有程式，在同一個 `engine.py` 上逐步演化，
就像 sections 的 `loop.py` 一章一章長大。
diff 相鄰兩個階段的 `src/`，看到的就是那個階段的機制。
...
- **只有介面、沒有保證：**三個動詞本身什麼都不保證。
  合約真正的內容是那條鐵律（事件留著、view 可重建），後面每個階段的測試就是在逐一驗它。

---

## 可執行程式

[`src/`](src/) 是整條鏈的起點，後面每個階段都會帶著它往下走：
...
- [第 9 章 · Memory](https://github.com/hardness1020/awesome-agent-architecture/tree/main/sections/09-memory/README.zh-TW.md)：Claude Code（按專案）和 Hermes（按使用者）的 scope 做法。
- [Production memory track](../README.zh-TW.md)：這一章實作的階段 1 和核心抽象。
