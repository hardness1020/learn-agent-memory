...
这一章讲 [Production memory](../README.zh-CN.md) track 里 lifecycle 的阶段 4（Encode）：
把通过 write gate（阶段 3）的 candidate，变成有类型、验证过的记录。
...
[第 9 章](https://github.com/hardness1020/awesome-agent-architecture/tree/main/sections/09-memory/README.zh-CN.md)分类的标准是「什么值得留」（user、feedback、project、reference）。
这个阶段改用「怎么用」来分：
...
验证跟阶段 3 是同一套模式：规则写死的检查，一出错就直接抛异常。
...
数据这样流：write gate（阶段 3）放行的 candidate 进到这个阶段，盖上 kind 和 epistemic type，变成正式的记录。
之后 resolution（阶段 5）补上 bitemporal 字段（事情何时为真、何时记下），supersede 时改写 `status`。
retrieval（阶段 8）按 kind 分路查询，context assembly（阶段 9）把 epistemic 标签印在内容旁边，
让模型看得出哪些是观察、哪些是系统自己的推论。

### What Changed

跟第 9 章比，变的是问题本身：那四种文件类型回答「值不值得留」，
kind 回答「要拿来做什么」，epistemic type 回答「我们怎么知道的」。
「值不值得留」的判断已经搬去阶段 3，这个阶段只管记录长什么样子。
...
- [第 9 章 · Memory](https://github.com/hardness1020/awesome-agent-architecture/tree/main/sections/09-memory/README.zh-CN.md)：这个阶段扩展的那四种「值得留」分类。
- [Production memory track](../README.zh-CN.md)：这个阶段所在的 lifecycle。
