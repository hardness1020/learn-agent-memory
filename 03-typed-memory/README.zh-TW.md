...
這一章講 [Production memory](../README.zh-TW.md) track 裡 lifecycle 的階段 4（Encode）：
把通過 write gate（階段 3）的 candidate，變成有類型、驗證過的紀錄。
...
[第 9 章](https://github.com/hardness1020/awesome-agent-architecture/tree/main/sections/09-memory/README.zh-TW.md)分類的標準是「什麼值得留」（user、feedback、project、reference）。
這個階段改用「怎麼用」來分：
...
驗證跟階段 3 是同一套模式：規則寫死的檢查，一出錯就直接拋例外。
...
資料這樣流：write gate（階段 3）放行的 candidate 進到這個階段，蓋上 kind 和 epistemic type，變成正式的紀錄。
之後 resolution（階段 5）補上 bitemporal 欄位（事情何時為真、何時記下），supersede 時改寫 `status`。
retrieval（階段 8）按 kind 分路查詢，context assembly（階段 9）把 epistemic 標籤印在內容旁邊，
讓模型看得出哪些是觀察、哪些是系統自己的推論。

### What Changed

跟第 9 章比，變的是問題本身：那四種檔案類型回答「值不值得留」，
kind 回答「要拿來做什麼」，epistemic type 回答「我們怎麼知道的」。
「值不值得留」的判斷已經搬去階段 3，這個階段只管紀錄長什麼樣子。
...
- [第 9 章 · Memory](https://github.com/hardness1020/awesome-agent-architecture/tree/main/sections/09-memory/README.zh-TW.md)：這個階段擴充的那四種「值得留」分類。
- [Production memory track](../README.zh-TW.md)：這個階段所在的 lifecycle。
