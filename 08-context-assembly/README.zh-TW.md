...
這一章講 [Production memory](../README.zh-TW.md) track 的 lifecycle 階段 9：
...
[第 9 章](https://github.com/hardness1020/awesome-agent-architecture/tree/main/sections/09-memory/README.zh-TW.md)已經把回想的文字包成 `<system-reminder>`，放在 user 訊息前面。
這個階段保留那條規則，再補上其他的：
...
階段 4 貼上的那兩組標籤（kind 和 epistemic status），在這裡做完最後一件事：
...
跟第 9 章比：那時候 recall 把分數最高的 k 筆內文原樣塞進去。
...
- [`engine.py`](src/engine.py)：`recall()` 補完了 contract（observe、consolidate、recall 現在都能離線端到端跑），
  並照階段 5 的 claim key 幫檢索結果填 `contradicts`。
  這裡不填的話，這個階段 render 出來的矛盾標籤，除了自己的測試以外永遠不會出現。
...
- [第 9 章 · Memory](https://github.com/hardness1020/awesome-agent-architecture/tree/main/sections/09-memory/README.zh-TW.md)：這個階段沿用的 `<system-reminder>` 包裝。
