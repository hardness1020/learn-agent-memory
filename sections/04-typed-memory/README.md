...
This page covers lifecycle section 4 of the [Production memory](../../README.md) track,
Encode: turning a candidate that survived the write gate (section 3) into a typed, validated record.
...
File-based agent memory usually types entries by what is worth keeping (user, feedback, project, reference).
This section types them by how they are used:
...
Validation is the same pattern as section 3: deterministic checks, loud failures.
...
Data flow: the write gate (section 3) emits stored candidates; this section constructs the record and stamps kind and epistemic status.
Resolution (section 5) later adds the bitemporal fields (when true, when recorded) and flips `status` on supersede.
Retrieval (section 8) routes queries by kind, and context assembly (section 9) prints the epistemic label,
so the model can see which claims are observations and which are the system's own inferences.

### What Changed

Compared with keep-worthiness typing, the question itself changed: those four file types answered "is this worth keeping".
Kind answers "how will this be used", and the epistemic axis answers "how do we know it".
Selection criteria moved into section 3; this section is purely about shape.
...
- **An inference is stored as fact.** The model's guess gets recited as truth. The epistemic type is mandatory
  at construction, and assembly (section 9) prints it next to the content.
...
- **Schema churn.** A new field or kind strands old records. Derived records rebuild from the ledger (section 2),
