...
lifecycle 的 Resolve，处理身份、冲突和时间。

第 4 章让每笔记录有了类型和出处，但没有回答一个问题：新记录跟旧记录讲的不一样时，该怎么办。
没有答案的存储区只剩两条路，而且都不好。
...
所有操作都不破坏数据。没有想改哪就改哪的 `UPDATE`，也没有 `DELETE`，event ledger（第 2 章）那边同样没有：

```text
ADD        新记录进来，生效中
SUPERSEDE  旧的关闭，新的开始
RETRACT    标记这笔是错的，但还读得到
ABSTRACT   多笔记录浓缩成一笔更高层的记录（第 6 章会产生）
```
...
数据怎么流：write gate（第 3 章）放行 candidate，第 4 章把它做成有类型的记录，
这一章把记录放上时间轴，并返回做了哪些操作。
engine 把每个操作写回 ledger 变成 event，所以时间轴本身也能重放。
retrieval（第 8 章）只捞生效中的记录；context assembly（第 9 章）会读这一章记下的冲突，
把两边都打出来，不会偷偷选一边。

### 整合时发现的两个 bug

这两个 bug 都出在 write gate（第 3 章），但要等这一章接上来、更正真的送进 gate，才看得出来。
这一章的第一版就是这样坏掉的。
照这条 track 的惯例，修正落在这一章带着走的那份 code，前面的文件夹保留当时的版本，对照着看就知道整合改了什么。
...
跟第 4 章比：以前一笔记录写好就定了，是一笔有类型、不会再动的数据。
...
- [Production memory track](../../README.zh-CN.md)：这一章所属的 lifecycle。
