...
This page covers lifecycle section 6 of the [Production memory](../../README.md) track,
...
Section 5 handled one narrow case, a new claim closing an old one. This section handles the rest,
and it is the first section where a model decides what happens to memories that already exist.
...
Rejections are returned, not swallowed, because section 10 measures consolidation precision from them.
...
the system's own derivation, and section 4's rule that inferences never silently become facts still holds.
...
Both bugs live in earlier sections, and both stayed invisible until this section ran against them.
...
every pair this section is able to merge is a pair the gate had already dropped.
...
if decision.action in (STORE, DEFER):
    ...
    # survive long enough for section 6 to see it
...
The gate scored similarity one way and this section grouped records by another.
The same pair could look similar enough for the gate to defer, then score below this
section's merge threshold, so everything the gate handed over sat in the store and
...
A handoff between two sections is only real when both sides measure "similar" the same way.

### What Changed

Compared with section 5, `consolidate()` is the contract's third verb, and it is now implemented.
`observe()` and `consolidate()` both work end to end offline; `recall()` arrives with section 8.
...
The proposer seam is where a model plugs in. The default one is deterministic, so this section has no `demo.py`.
