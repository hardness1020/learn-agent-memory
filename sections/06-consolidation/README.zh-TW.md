...
lifecycle 的 Consolidate，把 event 變成知識。
...
第 5 章只處理了其中很窄的一塊（新主張關掉舊主張），剩下的都在這一章，
而且這是第一次由模型決定已經存在的 memory 該怎麼處置。

難就難在這裡。在這之前，每一章都只會新增紀錄。
...
被退回的 Proposal 會回報出去，不會被吞掉，因為第 10 章要靠這些數字算 consolidation 的精準度。
...
這兩個 bug 都出在更早的章節，但要等這一章跑起來、跟前面串在一起，才看得出來。
照這條 track 的慣例，修正落在這一章帶著走的那份 code，
前面的資料夾保留當時的版本，對照著看就知道整合改了什麼。

**Bug 1：`DEFER` 沒有地方可去。**
write gate 會把跟現有 memory 很像的 candidate 延後處理，
理由寫著「已經有相似的 memory，交給 consolidation 合併」。
但被延後的 candidate 根本沒存下來，consolidation 也就永遠看不到它：這一章合併得動的每一組，gate 早就先丟掉了。
...
gate 判斷像不像用一種算法，這一章分組的時候用的是另一套。
同一對紀錄，gate 算出來夠像，所以把 candidate 延後過來；
這一章再算一次卻不夠像，達不到合併的門檻。
結果 gate 交過來的東西全部躺在儲存區裡，一筆都沒被合併過。
修法是這一章直接用 gate 的函式和門檻，兩邊量出來的永遠是同一個數字：
...
兩章要交接，得先對「像不像怎麼算」有共識，不然一邊說夠像，另一邊永遠算出不夠像。

### What Changed

跟第 5 章比：`consolidate()` 是 contract 的第三個動詞，現在有實作了。
`observe()` 和 `consolidate()` 都能離線跑完整條路；`recall()` 要等第 8 章。
...
模型要接進來，就是從 proposer 這個位置接。預設的 proposer 是確定性的，所以這一章沒有 `demo.py`。
