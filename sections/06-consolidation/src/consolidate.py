"""Consolidation (production memory track, section 6: turn events into knowledge).
...
ledger (section 2), so a bad consolidation pass is recoverable by replay.
"""
...
from records import KINDS, make_record, validate
...
# The gate defers a candidate at SIMILAR_AT and tells it to merge here, so the
# merge threshold has to be the same number measured the same way. When this
# module used jaccard and the gate used asymmetric overlap, everything the gate
# deferred landed below the merge line and stayed unmerged forever.
MERGE_AT = SIMILAR_AT
...
class Rejected(ValueError):
    """A proposal the validator refused. Carries the reason, because section 10
...
def check(proposal, store) -> list:
    ...
    Everything make_record would reject is checked here too. apply() mutates
    the store, so a proposal that fails halfway through leaves records and
    ledger disagreeing, and the whole pass is meant to be recoverable."""
    if proposal.level not in LEVELS:
        raise Rejected(f"unknown level: {proposal.level}")
    if proposal.kind not in KINDS:
        raise Rejected(f"unknown kind: {proposal.kind}")
    ...
    if len({r.claim_key for r in sources}) > 1:
        # Different keys mean different claims, however alike the wording. Merging
        # them would drop at least one key, and the profile view is keyed, so the
        # merged record would vanish from "who is this user" entirely.
        raise Rejected("merges records that make different claims")
    return sources
...
def apply(proposal, store, at) -> list:
    ...
    keys = {r.claim_key for r in sources if r.claim_key}
    derived = make_record(sources[0].scope, proposal.kind, "inference",
                          ...
                          # A merge that drops the claim key empties the profile
                          # view: the sources stop being active and the derived
                          # record has no key to project. It would also never
                          # supersede a future competing claim.
                          claim_key=keys.pop() if len(keys) == 1 else "",
                          # Without valid_from the merged claim reads as true at
                          # every past date, including before its evidence existed.
                          valid_from=at)
    store.add(derived)
...
def propose_compression(records) -> list:
    ...
    for i, record in enumerate(records):
        if record.id in used or record.kind != "semantic":
            continue
        # Group only within the record's own scope. A tenant-wide pass pools
        # every user, and a cross-scope group is rejected by check() after
        # used.update has already claimed its members, so the valid same-user
        # merge inside it never gets proposed, on this pass or any later one.
        own = (record.scope.tenant_id, record.scope.user_id)
        group = [record] + [r for r in records[i + 1:]
                            if r.id not in used and r.kind == record.kind
                            and (r.scope.tenant_id, r.scope.user_id) == own
                            and r.claim_key == record.claim_key
                            and resembles(record.content, r.content) >= MERGE_AT]
...
def _shortest(group) -> str:
    ...
    picking an existing sentence keeps this section offline and checkable."""
