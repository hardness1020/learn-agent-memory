"""Evaluation and governance (production memory track, section 10: Feedback).

The loop the first nine sections leave open. They write, resolve, merge, index,
rank, and inject, and none of them ever learns whether any of it was right.

This section owns no storage. Every number here is a view over the ledger
(section 2) and the records, the same rule section 7 applies to indexes: drop it
and recompute it. Nothing is measured that was not logged, which is why the
earlier sections log their rejections, their operations, the rule that decided,
...
Not every metric section 10 owes can be computed from a live log. Temporal
...
def write_quality(events, records) -> dict:
    """From section 3's decision log and section 6's rejections, both of which
...
def retrieval_quality(events, cases=()) -> dict:
    ...
    return {"evidence_precision": _mean(
            ...
            # section 9 takes a conflict group whole or not at all, so per turn
...
def context_quality(events, records) -> dict:
    ...
    stale = shown = 0
    for turn in turns:
        # what the system still believed at the moment it injected. section 5
    ...
    return {"injected_tokens": _mean([i.tokens for i in turns]),
            ...
            # which is section 9 working. whether the answer abstained is an
...
def injection_metadata(hits, selected) -> dict:
    """The fields section 10 measures the context layer from, built next to the
...
def looks_like_instruction(text) -> bool:
    """Stored text that reads like a command rather than a fact. Section 9 frames
...
def outcomes(events) -> list[Outcome]:
    ...
    section warns about one paragraph later."""
