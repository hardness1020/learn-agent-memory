"""Evaluation offline checks: the four layers, missing vs zero, and the gates.
    ...
    python sections/10-evaluation-governance/src/test.py
"""
import tempfile
...
def test():
    ...
    # write layer: decisions counted by action, from section 3's log
    ...
    # section 9 working. it is not the answer abstaining: that is an outcome
    ...
    # stale rate: measurable only because section 5 stamps superseded_at instead
    ...
    # contradiction coverage: section 9 takes a conflict group whole or not at
    ...
    # prompt-injection hits: section 9 frames memory as data, so a hit is not an
    ...
    # retraction is the one state change this section makes, and the only thing
    ...
    # consolidation precision: section 6 reports its rejections, so a proposer
    ...
    # conflict and injected neither side, which is section 9 giving up on a group
