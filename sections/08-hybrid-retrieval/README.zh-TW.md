...
這一章講的是 [Production memory](../../README.zh-TW.md) track lifecycle 的第 8 章（Retrieve）：
...
最小可行的 loop 已經有兩種查法：LLM selector 讀 index 挑 memory，
關鍵字搜原始歷史。這一章把它推廣成一條 pipeline：

1. 幾條輕量的 candidate 通道並行跑。
2. 把它們的排名融合起來，不用調跨通道的權重。
3. 要能降級：一條通道壞掉，結果不能跟著變空。
4. 要分得清：通道跑完但一筆都沒對上，跟通道壞掉，是兩回事。
5. index 要一直可以重建：它是 view（第 7 章），從紀錄推導出來，紀錄才是本體。
...
| --- | --- | --- |
| **做法** | 命中之後，回 event ledger（第 2 章）把那筆前後的 event 一起撈出來。 | 把執行軌跡存成檔案，讓 coding agent 在 sandbox 裡自己搜。 |
...
跟前後章節怎麼接：index 從有類型的紀錄（第 4 章）推導出來，紀錄在寫入時就標好 scope，
所以 scope 過濾在通道開跑之前就做完了。index 的維護留在寫入路徑（第 7 章的拆法），查詢只讀不寫。
index 壞了，把 ledger 重放一次就回來（第 2 章的保證）。
融合出來的命中會送去 context assembly（第 9 章）。
每筆命中連同它的類型、分數、記錄時間和來源 event id 一起送過去，不是只送一段文字。

### What Changed

跟最小可行的 loop 比：那時候的 recall 一次只走一條路，走哪條看是誰觸發的。
...
- **k 一大精準度就崩：**candidate 越多，下游雜訊越多。
  融合時用小 k，最後一刀交給 assembly 的 budget（第 9 章）去切。
- **延遲越積越多：**通道要真的並行跑才算並行。
...
- [`retrieve.py`](src/retrieve.py)：RRF 的 `fuse`，和把第 7 章那幾條通道融合起來的 `retrieve`。
  index 本身放在 [7 · Index views](../07-index-views/README.zh-TW.md)。
