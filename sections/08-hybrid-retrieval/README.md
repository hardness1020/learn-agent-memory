...
This page covers lifecycle section 8 of the [Production memory](../../README.md) track, Retrieve:
...
The minimum loop already retrieves two ways: an LLM selector over the index,
and keyword search over raw history. This section generalizes that into a pipeline:
...
5. Keep the index rebuildable, because it is a view (section 7): derived from the records, never the source of truth.
...
merge + dedupe, rerank               RRF fusion
    ↓
source-context expansion             (live: pull surrounding events, section 2)
    ↓
token-budget selection               section 9's job
```
...
| --- | --- | --- |
| **What it does** | Pull the events around a hit from the event ledger (section 2). | Store trajectories as files; a coding agent searches them in a sandbox. |
...
How it integrates: the index derives from typed records (section 4), and records carry scope
stamped at write time, so scope filtering happened before any channel runs.
Index maintenance stays on the write path (section 7's split), so a query only reads.
Rebuilding after corruption is one replay (section 2's guarantee).
The fused hits go to context assembly (section 9), each carrying its kind, score,
recorded time, and source event ids, not as bare strings.

### What Changed

In the minimum loop, recall was one channel at a time, chosen by who triggered it.
...
- **Precision collapses at high k.** More candidates means more noise downstream. Fuse at small k and let
  assembly's budget (section 9) do the final cut.
...
- [`retrieve.py`](src/retrieve.py): RRF `fuse` and the fused `retrieve` over section 7's channels.
  The index itself now lives in [7 · Index views](../07-index-views/).
...
```bash
python sections/08-hybrid-retrieval/src/test.py   # offline checks, no key
```
