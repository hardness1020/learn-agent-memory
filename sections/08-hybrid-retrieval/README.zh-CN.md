...
这一章讲的是 [Production memory](../../README.zh-CN.md) track lifecycle 的第 8 章（Retrieve）：
...
最小可行的 loop 已经有两种查法：LLM selector 读 index 挑 memory，
关键字搜原始历史。这一章把它推广成一条 pipeline：

1. 几条轻量的 candidate 通道并行跑。
2. 把它们的排名融合起来，不用调跨通道的权重。
3. 要能降级：一条通道坏掉，结果不能跟着变空。
4. 要分得清：通道跑完但一笔都没对上，跟通道坏掉，是两回事。
5. index 要一直可以重建：它是 view（第 7 章），从记录推导出来，记录才是本体。
...
| --- | --- | --- |
| **做法** | 命中之后，回 event ledger（第 2 章）把那笔前后的 event 一起捞出来。 | 把执行轨迹存成文件，让 coding agent 在 sandbox 里自己搜。 |
...
跟前后章节怎么接：index 从带类型的记录（第 4 章）推导出来，记录在写入时就打好了 scope，
所以 scope 过滤在通道开跑之前就做完了。index 的维护留在写入路径（第 7 章的拆法），查询只读不写。
index 坏了，把 ledger 重放一次就回来（第 2 章的保证）。
融合出来的命中会送去 context assembly（第 9 章）。
每笔命中连同它的类型、分数、记录时间和来源 event id 一起送过去，不是只送一段文字。

### What Changed

跟最小可行的 loop 比：那时候的 recall 一次只走一条路，走哪条看是谁触发的。
...
- **k 一大精度就崩：**candidate 越多，下游噪声越多。
  融合时用小 k，最后一刀交给 assembly 的 budget（第 9 章）去切。
- **延迟越积越多：**通道要真的并行跑才算并行。
...
- [`retrieve.py`](src/retrieve.py)：RRF 的 `fuse`，和把第 7 章那几条通道融合起来的 `retrieve`。
  index 本身放在 [7 · Index views](../07-index-views/README.zh-CN.md)。
