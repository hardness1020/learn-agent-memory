...
This page continues the [Production memory](../../README.md) track: lifecycle section 2, Capture,
the append-only event log every later section reads from.
...
The mini version is one session log: append each run's text to SQLite at run end,
...
Compared with a plain session log:

- A session log keys rows by session. The ledger keys them by scope, so tenants share one table without sharing rows.
- `event_type` widens capture beyond chat: tool results, corrections, approvals, and outcomes are evidence too.
- `occurred_at` and `recorded_at` split world time from system time, the base for temporal resolution (section 5).
...
- **Fabricated world time.** Guessing `occurred_at` poisons temporal resolution later (section 5).
...
- [Production memory track](../../README.md): the full lifecycle this section belongs to.
