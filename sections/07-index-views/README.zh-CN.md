...
这一章讲 [Production memory](../../README.zh-CN.md) track 的第 7 章（Index）。
view 指的是从记录整理出来的读取结构，搜索用的 index、看当下状态的 profile、
互相链接的 wiki 页面都是。每一种都从同一份 ledger 衍生出来。

走到这一章，一笔记录已经有类型、有时间轴、也有合并的历史，但它终究只是一行数据。
不同的问题，要的答案长得完全不一样。
...
这个单向流动就是这一章要守的规则。index 坏掉不算事故，重建就好：
...
第 8 章起每次写入跑完都会重建一次，所以这不是偶发事故：谁最后写，表里就只剩谁的数据。
...
profile 是第 5 章的 claim key（一个事实更新时固定用的那个名字）第二次派上用场的地方。
没有 key 就没有投影的目标，「当下状态」就会退化成「最近那几笔 memory」。

数据的流向：记录进去，view 出来，engine 随时可以重建任何一种。
`reindex` 拿一个 scope 还生效的记录，把 sparse view 从头重写一次。
这一章的 src 只给出 `reindex` 这个动作，没有任何路径会自动调用它。
到了第 8 章，换写入路径来调用：任何改动记录的流程跑完就 reindex 自己的 scope，retrieve 只读表。
...
这两个前提都不成立，所以这一章两种都没做。

### What Changed

和第 6 章相比，建 index 和做检索分成两件事。
这一章建出返回 candidate 的各个通道，第 8 章才决定怎么跨通道排序。
